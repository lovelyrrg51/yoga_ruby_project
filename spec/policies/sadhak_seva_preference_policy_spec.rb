RSpec.describe SadhakSevaPreferencePolicy, type: :policy do
  ...
  permissions :create?, :update? do
    ...
    it 'grants access if user and sadhak_seva_preference has same sadhak profile' do
      sadhak_profile = SadhakProfile.new
      user = User.new super_admin: false, digital_store_admin: false,
        sadhak_profile: sadhak_profile
      record = SadhakSevaPreference.new sadhak_profile: sadhak_profile
      expect(subject).to permit(user, record)
    end

    it 'grants access if user sadhak profiles include sadhak_seva_preference sadhak profile' do
      sadhak_profile = SadhakProfile.new
      user = User.new super_admin: false, digital_store_admin: false,
    ...
    it 'denies access if user is not super/digital_store admin and does not share the same sadhak profile' do
      ...
      record = SadhakSevaPreference.new sadhak_profile: SadhakProfile.new
