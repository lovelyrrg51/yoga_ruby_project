RSpec.describe SyClubSadhakProfileAssociationPolicy, type: :policy do
  subject { described_class }

  permissions :create?, :update? do
    it 'grants access if user is super_admin' do
      user = User.new super_admin: true, club_admin: false
      record = SyClubSadhakProfileAssociation.new sy_club: SyClub.new
    ...
    it 'grants access if user created sy_club_digital_arrangement_detail forum' do
      user = User.new super_admin: false, club_admin: false
      record = SyClubSadhakProfileAssociation.new sy_club: SyClub.new(user: user)
