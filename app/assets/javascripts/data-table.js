function dataTableDefaultConfigForNoAjax() {
    return {
        searching: true,
        autoWidth: false,
        filter: true,
...
$(document).on('turbolinks:load', function() {
    ...
    $("table.datatable-no-ajax").each(function() {
        var dataTableDefaultConfiguration = dataTableDefaultConfigForNoAjax();
        if ( $(window).width() < 767 ){
            dataTableDefaultConfiguration["pagingType"] = "simple";
        }
        var config = $(this).data('config');
        ...
        var table = $(this).DataTable($.extend(true, dataTableDefaultConfiguration, {}));
    });
});
