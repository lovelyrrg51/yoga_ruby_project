class PaymentRefundsController < ApplicationController
  
  before_action :authenticate_user!, except: []
  before_action :set_payment_refund, only: [:update, :refund]
   ...
   # POST /payment_refunds/refund
    def refund
      ...
      begin
        ...
        begin
          ...
          Global.instance.send(:current_user=, @payment_refund.requester_user)
          @event_order.pay_or_refund({sadhak_profiles: sadhak_profiles, event_id: request_object[:shifted_event_id] })

        ensure
          Global.instance.send(:current_user=, send(:current_user))
        end
    ...
    def update
      ...
      if @payment_refund.update(payment_refund_params)
        ...
        flash[:error] =  @payment_refund.errors.full_messages.first
      end
      redirect_back(fallback_location: proc { root_path })
    end
